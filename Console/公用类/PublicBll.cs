...
namespace ConsoleHydee
{
    public class PublicBll
    {
        ...
        public string ReturnHead_hydee(bool ifcommit, List<RetHeadJson> _RetHeadJsont, string ReqHeadJson, string RetDataJson, string ReqBodyJson, string funccode, string ls_connect)
        {
            ...
            if(_RetHeadJsont[0].returncode == "0000")
            {
                ls_status = "1";
            }
            else
            ...
            if (ls_status == "2" && funccode == "8")
            { }
            else
            {
                dao.WriteLog("168", DateTime.Now, ReqHeadJson, _RetHeadJsont[0].returnmsg, null, null, null, ls_connect, funccode, ls_status);
            }
        ...
        public string interface_xyf(string ls_sendtext, string ls_param1, string ls_param2, string ls_param3, string ls_param4, string ls_param5)
        {
            ...
            string ls_sql = "";
            string ls_op_shop_id = "";
            DataTable dt = null;
            DataTable dtnew = null;

            dt = dao.GetDataTable("select nvl(op_shop_id,'') as op_shop_id,nvl(time_start,'') as time_start,nvl(time_expire,'') as time_expire,nvl(attach,'') as attach,nvl(body,'') as body,nvl(total_fee,'') as total_fee,nvl(mch_create_ip,'') as mch_create_ip,nvl(auth_code,'') as auth_code from interface_gjzh_fkmsk where out_trade_no = '" + ls_sendtext + "'");
            if (dt.Rows.Count == 0)
            {
                ls_retmsg = "无法识别的订单号！";
                return ls_retmsg;
            ...
            ls_op_shop_id = dt.Rows[0]["op_shop_id"].ToString();
            ...
            this.reqHandler.setParameter("op_shop_id", ls_op_shop_id);//门店编号
                        ...
                        if (int.Parse(param["status"].ToString()) != 0)
                        {
                            ...
                            if (param["need_query"] != null)
                            {
                                ...
                                else
                                {
                                    ...
                                    ls_sql = "update interface_gjzh_fkmsk set MESSAGE = '未知情况',STATUS = '0' where out_trade_no = '" + ls_sendtext + "'";
                                    dao.SqlDataTableCommit(ls_sql);
                                }
                            }
                            return ls_retmsg;
        ...
        #region 亿保上传目录
        public string interface_yb_sc(string reqdata, string ls_param1, string ls_param2, string ls_param3, string ls_param4, string ls_param5)
        {
            string ls_retmsg = "";
            this.reqHandler = new RequestHandler(null);
            //this.cfg = Utils.loadCfg(); ;
            Dictionary<string, string> reqContent = new Dictionary<string, string>();
            this.reqHandler.setGateUrl_ybsc(ls_param5);
            reqContent.Add("url", this.reqHandler.getGateUrl_ybsc());
            reqContent.Add("data", reqdata);
        ...
        #region 售药机补、退货回调
        public string interface_syj_hd(string reqdata, string ls_param1, string ls_param2, string ls_param3, string ls_param4, string ls_param5)
        {
            string ls_retmsg = "";
            this.reqHandler = new RequestHandler(null);
            Dictionary<string, string> reqContent = new Dictionary<string, string>();
            this.reqHandler.setGateUrl_syjhd(ls_param5);
            reqContent.Add("url", this.reqHandler.getGateUrl_syjhd());
            //reqContent.Add("contenttype", "application/json");
            reqContent.Add("data", reqdata);
